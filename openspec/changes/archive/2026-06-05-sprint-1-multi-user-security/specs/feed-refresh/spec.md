...
- **THEN** the system starts fetching new articles from all feeds owned by the current user in background and shows a loading indicator

#### Scenario: New articles appear after refresh
- **WHEN** the background refresh completes and new articles are found for the current user's feeds
