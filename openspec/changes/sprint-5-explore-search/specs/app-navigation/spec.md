...
### Requirement: Navigate via bottom navigation bar
...
- **THEN** `page.push_route("/")` is called and the explore view is displayed

#### Scenario: Navigate via bottom navigation bar
- **WHEN** user taps a destination on the NavigationBar
- **THEN** `page.push_route()` is called with the corresponding route. Index 0 ("Início") SHALL navigate to `/` (explore view). Index 1 ("Feeds") SHALL navigate to `/feeds` (feed list view).
