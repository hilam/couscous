...
async def test_entry_list_view_route():
    ...
    with (
        patch("app.views.entry_list_view.get_db_session", return_value=cm),
        patch("app.views.entry_list_view.list_entries", AsyncMock(return_value=[])),
    ):
        view = await entry_list_view(page, state)
