...
async def test_entry_view_route():
    ...
    with (
        ...
        view = await entry_view(ctx, entry_id=1)
...
async def test_entry_view_contains_star_button():
    ...
    star_buttons = [
        b
        for b in (app_bar.actions or [])
        if isinstance(b, ft.IconButton) and b.icon == ft.Icons.STAR_BORDER
    ]
...
async def test_entry_view_not_found():
    ...
    with patch("app.views.entry_view.get_entry", AsyncMock(return_value=None)):
        view = await entry_view(ctx, entry_id=999)
